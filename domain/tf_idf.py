...
Original file is located at
    https://colab.research.google.com/drive/1R7jjTbXBlAY6ZBhgp7e4qJHgHiOPKSaP
...
file_root = "/Users/river1440/Desktop/Text Mining 2022/STARS_API/test_file"
...
def tokenize(description) -> list:
    tokens = description.split()
    return tokens

def stemming(itokens) -> list:
    ps = PorterStemmer()
    tokens = [ps.stem(word) for word in itokens]
    return tokens

def remove_stopwords(itokens) -> list:
...
def group_by_levels(df) -> dict: # O(n*k) n:length_of_df ; k:length_of_levels
...
df_in['tokens'] = [remove_stopwords(stemming([clean_word(j) for j in tokenize(i)])) for i in df_in['Description']]
print(df_in['tokens'])

dict_levels = group_by_levels(df_in)
print(dict_levels)
...
for index, row in tf_dataframe.iterrows():
  tf_idf_token_list[index] = list(row.sort_values(ascending=False)[:49].to_dict().keys())

# for each category, get first 50 token
# give json
import json
tf_idf_token_json = json.dumps(tf_idf_token_list, indent = 4)
print(f"tf_idf_token_json: {tf_idf_token_json}")